...
import { Navigation, Plus, MapPin, X, Map, School, Hospital, Train, ShoppingCart, ShoppingBag, Landmark, Coffee, Utensils, Droplet, User, Home, Sparkles, Building2, Briefcase, Ruler, Zap, ShieldCheck } from "lucide-react";
    ...
    return (
        ...
        >
            ...
            <div className="space-y-8">
                {/* Add Landmark Form */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-zinc-50 p-6 rounded-3xl border border-dashed border-zinc-200">
                    ...
                    <div className="md:col-span-1 space-y-2 group">
                        <label className="text-[11px] font-black text-brand-paragraph uppercase tracking-widest px-1">Place Name</label>
                        <input
                            type="text"
                            placeholder="e.g. Kharghar Stn"
                            value={placeName}
                            onChange={(e) => setPlaceName(e.target.value)}
                            className="w-full bg-white border border-brand-muted/30 rounded-2xl px-5 py-3 text-[13px] font-semibold focus:outline-none focus:border-brand-primary"
                        />
                    </div>

                    <div className="md:col-span-1 space-y-2 group">
                        <label className="text-[11px] font-black text-brand-paragraph uppercase tracking-widest px-1">Distance (KM)</label>
                        <div className="relative">
                            <input
                                ...
                                className="w-full bg-white border border-brand-muted/30 rounded-2xl px-5 py-3 text-[13px] font-semibold focus:outline-none focus:border-brand-primary pr-12"
                            />
                    ...
                    <div className="md:col-span-1 flex items-end">
                        <button
                            ...
                            className="w-full h-[46px] bg-brand-primary text-white rounded-2xl text-[12px] font-black uppercase tracking-widest hover:bg-brand-primary/90 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-brand-primary/20 disabled:opacity-50 disabled:shadow-none"
                        >
                        ...
                        {(formData.nearbyPlaces || []).map((place, index) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="bg-white border border-zinc-100 p-4 rounded-3xl flex items-center gap-4 shadow-sm hover:shadow-md transition-all group"
                                >
